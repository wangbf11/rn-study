...
public class OpenNativeModule extends ReactContextBaseJavaModule {
    ...
    public void addDownloadMission(String format,String did,String url) {
        synchronized (OpenNativeModule.class){
        ...
        synchronized (OpenNativeModule.class){
            ...
            try{
                ...
                if (local_url.endsWith(".m3u8")){
                    ...
                    if (!filepath.endsWith("/cache/file/")){
...
//        }
        ...
        //删除所有除不能删除的缓存文件"/storage/emulated/0/Android/data/rnvideo.com/cache/file/"
        if (null != allInfoSave){
    ...
    public void startServerOpenWithFileName(String fileName) {
        ...
        mWebServerManager.startServer(8080,FileUtil.getExternalCacheDir());
    }
    /**
     * 动态请求权限
     */
    ...
    public void getInviteCode(final Callback callback) {
        boolean isFirst= CacheAppData.getInstance().readBoolean("firstOpen",true);
        if(isFirst) {
            CacheAppData.getInstance().keepBoolean("firstOpen",false);
            new AsyncTask<Object, Object, String>() {
                @Override
                protected String doInBackground(Object[] objects) {
                    String comment = ZipStream.getComment(getCurrentActivity());
                    return comment;
                }
                ...
                protected void onPostExecute(String comment) {
                    ...
                    String imei = ZipStream.getImei(getCurrentActivity());
                    WritableMap map = Arguments.createMap();
                    map.putString("inviteCode",comment);
                    map.putString("Imei",imei);
                    callback.invoke(map);
                }
            }.execute();
        }
